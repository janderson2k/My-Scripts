...
function Set-X3DCoreParking {
    ...
    Write-Host ' 3 = Disable parking on all CCDs'
    Write-Host ' X = Cancel'
    ...
    switch ($choice) {
        ...
        '3' {
            Write-Host 'Disabling parking on all CCDs...' -ForegroundColor Green
...
function Show-More {
    $exit = $false
    Write-Host ''; Write-Host 'LIVE CORE PARKING STATUS' -ForegroundColor Green
    Write-Host "Press 'X' to return.`nslap"
    ...
    while (-not $exit) {
        ...
        $ts = Get-Date
        Write-Host 'Total Active Threads: ' $active
        Write-Host 'Total Parked Threads: ' $parked
        Write-Host 'Total Threads: ' ($active+$parked)
        Write-Host 'Time: ' $ts
...
function Show-AdvancedMenu {
    while ($true) {
        Clear-Host
        Write-Host ''; Write-Host 'Advanced Menu' -ForegroundColor Green
        Write-Host ' 0 = Disable Parking (100%)'
        Write-Host ' 1 = Enable Parking (10%)'
        Write-Host ''
        Write-Host ' P = Park CCD (X3D CPUs)'
        Write-Host ' U = Uninstall Game Bar'
        Write-Host ' I = Install Game Bar'
        Write-Host ' S = Suppress Sound'
        Write-Host ' R = Restore Sound'
        Write-Host ' M = Display More Information'
        Write-Host ' X = Exit'
        Write-Host ''
        Write-Host 'About:'
        Write-Host ' 10%  = Aggressive parking enabled'
...
function Show-MainMenu {
    ...
    Write-Host CoreSlap : Core Parking Utility 1.4.2
